...
def create_dataframe(path_list, password):
    import threading
    threads : th.Thread = [threading.Thread(target=load_data_with_key, args=(p, password)) for p in path_list]
...
def filter_df(df, start, end, callback):
    dt_col : pd.Series = pd.to_datetime(df.iloc[:, 5].str.split("|").str[1], format='%m/%d/%Y %I:%M:%S %p')
    valid : pd.Series = (dt_col >= start) & (dt_col <= end) & df.iloc[:, 12:].all(axis=1)
